...
3. Authentication — does this page require login? (yes / no)
   If yes → login route, email, password, destination route after login
...
1. `mcp__playwright_headed__browser_navigate` to the login URL
2. `mcp__playwright_headed__browser_snapshot` — identify email/username field, password field, submit button by `ref=`
3. `mcp__playwright_headed__browser_type` email → `mcp__playwright_headed__browser_type` password → `mcp__playwright_headed__browser_click` submit
4. `mcp__playwright_headed__browser_snapshot` — verify navigation away from login
5. If still on login page: continue and note "Auth status: unconfirmed" in spec
